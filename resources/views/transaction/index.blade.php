<x-layout.default>
    <div class="py-6" x-data="{ activeTab: '{{ $activeTab }}', showCompleteModal: false, selectedTransaction: null }">
        <!-- Tab Navigation -->
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap flex items-center space-x-2">
                                        ...
                                        @if ($order->status !== 'completed')
                                            ...
                                            <button @click="showCompleteModal = true; selectedTransaction = { id: {{ $order->id }}, type: 'order', customer_name: '{{ $order->customer_name }}', service: '{{ $order->printing->nama_layanan ?? 'Layanan tidak ditemukan' }}', total_price: '{{ number_format($order->total_price, 0, ',', '.') }}' }"
                                                class="text-green-600 hover:text-green-900" title="Tandai sebagai Selesai">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap flex items-center space-x-2">
                                        ...
                                        @if ($purchase->status !== 'completed')
                                            ...
                                            <!-- Tombol Mark as Completed -->
                                            <button @click="showCompleteModal = true; selectedTransaction = { id: {{ $purchase->id }}, type: 'purchase', customer_name: '{{ $purchase->customer_name }}', service: '{{ $purchase->product->name ?? 'Produk tidak ditemukan' }}', total_price: '{{ number_format($purchase->total_price, 0, ',', '.') }}' }"
                                                class="text-green-600 hover:text-green-900" title="Tandai sebagai Selesai">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div x-show="showCompleteModal" x-cloak class="fixed z-50 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div x-show="showCompleteModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
                <div x-show="showCompleteModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                ...
                                <div class="mt-4 bg-gray-50 p-4 rounded-md">
                                    <p class="text-sm font-medium text-gray-700">Detail Transaksi:</p>
                                    <div class="mt-2 space-y-1">
                                        <p class="text-sm text-gray-600" x-text="'Pelanggan: ' + (selectedTransaction?.customer_name || '')"></p>
                                        <p class="text-sm text-gray-600" x-text="selectedTransaction?.type === 'order' ? 'Layanan: ' + (selectedTransaction?.service || '') : 'Produk: ' + (selectedTransaction?.service || '')"></p>
                                        <p class="text-sm text-gray-600" x-text="'Total: Rp ' + (selectedTransaction?.total_price || '0')"></p>
                                    </div>
                                </div>
                    ...
                    <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                        <form :action="selectedTransaction ? '/transaction/' + selectedTransaction.id + '/mark-completed' : ''" method="POST">
                            @csrf
                            @method('PATCH')
