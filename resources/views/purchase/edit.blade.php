            ...
            <form action="{{ route('purchase.update', $purchase->id) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="text" id="customer_name" name="customer_name"
                                ...
                                placeholder="Masukkan nama pelanggan" required value="{{ old('customer_name', $purchase->customer_name) }}">
                            @error('customer_name')
                        ...
                        <div>
                            ...
                            <input type="text" id="customer_phone" name="customer_phone"
                                class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                placeholder="Masukkan nomor telepon" value="{{ old('customer_phone', $purchase->customer_phone) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="email" id="customer_email" name="customer_email"
                            class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                            placeholder="Masukkan email pelanggan" value="{{ old('customer_email', $purchase->customer_email) }}">
                        ...
                        <select id="product_id" name="product_id"
                            ...
                            @foreach ($product as $produk)
                                <option value="{{ $produk->id }}" data-price="{{ $produk->price_per_unit }}"
                                    {{ old('product_id', $purchase->product_id) == $produk->id ? 'selected' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <select id="payment_method" name="payment_method"
                                ...
                                <option value="cash" {{ old('payment_method', $purchase->payment_method) == 'cash' ? 'selected' : '' }}>
                                    Cash</option>
                                <option value="transfer" {{ old('payment_method', $purchase->payment_method) == 'transfer' ? 'selected' : '' }}>
                                    Transfer</option>
                                <option value="credit_card" {{ old('payment_method', $purchase->payment_method) == 'credit_card' ? 'selected' : '' }}>
                                    Kartu Kredit</option>
                                <option value="debit_card" {{ old('payment_method', $purchase->payment_method) == 'debit_card' ? 'selected' : '' }}>
                                    Kartu Debit</option>
                            </select>
                        ...
                        <div>
                            <label for="paid_at" class="block text-gray-700 font-medium mb-2">Tanggal Pembayaran</label>
                            <input type="datetime-local" id="paid_at" name="paid_at"
                                class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                value="{{ old('paid_at', $purchase->paid_at) }}">
                            @error('paid_at')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="status" name="status"
                            ...
                            <option value="pending" {{ old('status', $purchase->status) == 'pending' ? 'selected' : '' }}>
                                Pending</option>
                            <option value="processing" {{ old('status', $purchase->status) == 'processing' ? 'selected' : '' }}>
                                Processing</option>
                            <option value="completed" {{ old('status', $purchase->status) == 'completed' ? 'selected' : '' }}>
                                Completed</option>
    ...
    <script>
        function purchaseForm() {
            return {
                ...
                quantity: {{ old('quantity', $purchase->quantity) }},
                productPrice: 0,
                totalPrice: 0,
