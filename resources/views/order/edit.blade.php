        ...
        <form action="{{ route('order.update', $order->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                    ...
                    >
                        ...
                        @foreach($printing as $cetak)
                            <option 
                                ...
                                {{ (old('printing_id', $order->printing_id) == $cetak->id ? 'selected' : '') }}
                            >
                ...
                <div class="mb-4">
                    ...
                    @if($order->file_path)
                        <p class="text-sm text-blue-500 mt-1">File saat ini: 
                            <a href="{{ asset('storage/' . $order->file_path) }}" target="_blank" class="underline">
                                Lihat File
                            </a>
                        </p>
                    @endif
...
<script>
    function orderForm() {
        return {
            selectedService: '{{ old('printing_id', $order->printing_id) }}',
            quantity: {{ old('quantity', $order->quantity) }},
            width: {{ old('width', $order->width) }},
            height: {{ old('height', $order->height) }},
            servicePrice: 0,
            totalPrice: 0,
